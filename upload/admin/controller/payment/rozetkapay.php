...
class ControllerPaymentRozetkaPay extends Controller {
    ...
    private function save(&$data) {
        ...
        if($this->config->get($this->prefix.'rozetkapay_test_status') == "1"){
            setcookie('rozetkapay_test_status', "1", time() + 24*7*60*60, "/");
        }else{
            if(isset($_COOKIE['rozetkapay_test_status'])){
                unset($this->request->cookie['rozetkapay_test_status']);
                unset($_COOKIE['rozetkapay_test_status']);
            }
        }
